...
function mouseClickSelect(){
	...
	$(document).one('click', function(e){
		...
		tagName2 = getTagName(target);
	  xpath = getQuery($(target));
	
		selector = getSelector();
		attributes = getAttrs(target);
		html = getHtml(target);
		result =  "{'tagName':\"" + tagName2 + "\",'xpath':\"" + xpath + "\", 'selector': \"" + selector + "\", 'attributes': {" + attributes + "}, 'html': \"" + html + "\"}";
	
		return false; // 使超链失效
	});

//	alert("xpath2"+xpath);
	return "{'tagName':\"" + tagName2 + "\",'xpath':\"" + xpath + "\", 'selector': \"" + selector + "\", 'attributes'= {" + attributes + "}, 'html': \"" + html + "\"}";
}
